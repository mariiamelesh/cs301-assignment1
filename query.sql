with user_amount_by_organization as (
	...
	select users.user_id, role, big_organizations.industry, count(login_logs) as failed_login_count
	...
	(select big_organizations.industry, systems.os_type, count(incident_id) as incident_count
	...
	union all
	(select big_organizations.industry, systems.os_type, count(event_id) as incident_count
